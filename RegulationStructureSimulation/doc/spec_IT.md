Entità: RegulationStructureSimulation  
=====================================  
...
Descrizione globale: **Questa entità contiene una descrizione armonizzata di un modello di dati per la simulazione della struttura di regolazione, per il dominio Raw-Water (Open Channels) System Management.**  
versione: 0.0.1  

## Elenco delle proprietà  

- `address`: L'indirizzo postale  - `alternateName`: Un nome alternativo per questa voce  - `areaServed`: L'area geografica in cui viene fornito un servizio o un articolo offerto  - `dataProvider`: Una sequenza di caratteri che identifica il fornitore dell'entità di dati armonizzata.  - `dateCreated`: Timestamp di creazione dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `dateModified`: Timestamp dell'ultima modifica dell'entità. Questo sarà di solito assegnato dalla piattaforma di archiviazione.  - `description`: Una descrizione di questo articolo  - `endSimulationTime`: Ora del giorno in formato ISO8601 UTC in cui termina la simulazione.  - `equivalentSluiceOpening`: Apertura equivalente della paratoia nel caso di paratoie multiple, stimata come valore medio delle diverse aperture.  - `estimatedGateDischargeCoefficient`: Coefficiente di scarico calibrato della paratoia.  - `id`: Identificatore unico dell'entità  - `initialConditions`: Descrizione dell'insieme delle modifiche da applicare alla struttura del regolamento per la simulazione.  - `inputParameters`: Descrizione dell'insieme delle modifiche da applicare alla struttura del regolamento per la simulazione.  - `location`: Riferimento Geojson all'elemento. Può essere Point, LineString, Polygon, MultiPoint, MultiLineString o MultiPolygon  - `modelError`: Errore percentuale tra lo scarico osservato e quello simulato.  - `modelledDischarge`: Scarico stimato dal modello di simulazione.  - `name`: Il nome di questo articolo.  - `owner`: Una lista contenente una sequenza di caratteri codificata in JSON che si riferisce agli ID unici dei proprietari  - `seeAlso`: elenco di uri che puntano a risorse aggiuntive sull'elemento  - `simulationOutput`: Descrizione dell'insieme dei risultati della simulazione della struttura di regolazione.  - `source`: Una sequenza di caratteri che dà la fonte originale dei dati dell'entità come URL. Si raccomanda di essere il nome di dominio completamente qualificato del fornitore di origine, o l'URL dell'oggetto di origine.  - `spillwayFlow`: Rapporto tra lo scarico dello sfioratore e il nuovo scarico totale  - `startSimulationTime`: Ora del giorno in formato ISO8601 UTC in cui inizia la simulazione.  - `targetDischarge`: Scarico desiderabile da stabilire nel canale, definito dagli operatori del servizio.  - `type`: Tipo di entità NGSI-LD. Deve essere RegulationStructureSimulation.    
...
#### RegulationStructureSimulation NGSI-v2 valori chiave Esempio  
Ecco un esempio di una RegulationStructureSimulation in formato JSON-LD come key-values. Questo è compatibile con NGSI-v2 quando si usa `options=keyValues` e restituisce i dati di contesto di una singola entità.  
...
```  
#### RegulationStructureSimulation NGSI-v2 normalizzato Esempio  
Ecco un esempio di una RegulationStructureSimulation in formato JSON-LD normalizzato. Questo è compatibile con NGSI-v2 quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
...
```  
#### RegulationStructureSimulation NGSI-LD valori-chiave Esempio  
Ecco un esempio di una RegulationStructureSimulation in formato JSON-LD come key-values. Questo è compatibile con NGSI-LD quando si usa `options=keyValues` e restituisce i dati di contesto di una singola entità.  
...
```  
#### RegulationStructureSimulation NGSI-LD normalizzato Esempio  
Ecco un esempio di una RegulationStructureSimulation in formato JSON-LD normalizzato. Questo è compatibile con NGSI-LD quando non usa opzioni e restituisce i dati di contesto di una singola entità.  
